...
import {
  ...
  CloudUpload, Zap, Trash2, Plus, LogOut, Mail, Lock, Loader2, Tag, Layers3,
} from "lucide-react";
...
/* Persist small bits of "where was I" state to sessionStorage so an in-progress
   flashcard/practice session survives an accidental page reload — e.g. a mobile
   browser discarding a backgrounded tab, or (previously) a spurious Supabase
   token-refresh reload. sessionStorage is scoped to this tab and cleared when the
   tab is closed, so it never leaks between devices or persists forever. */
function readSessionSlot(key) {
  try {
    const raw = sessionStorage.getItem(key);
    return raw != null ? JSON.parse(raw) : null;
  } catch (e) { return null; }
}
function writeSessionSlot(key, value) {
  try {
    if (value === null || value === undefined) sessionStorage.removeItem(key);
    else sessionStorage.setItem(key, JSON.stringify(value));
  } catch (e) { /* storage unavailable — degrade gracefully, no resume */ }
}
function clearSessionSlot(key) { writeSessionSlot(key, null); }

/* Drop-in replacement for useState whose value also mirrors into sessionStorage
   under `key`, and rehydrates from there on first mount (falling back to
   `initialValue` when nothing saved, or when the saved JSON is corrupt). */
function useResumableState(key, initialValue) {
  const [state, setState] = useState(() => {
    const saved = readSessionSlot(key);
    return saved !== null ? saved : (typeof initialValue === "function" ? initialValue() : initialValue);
  });
  useEffect(() => { writeSessionSlot(key, state); }, [key, state]);
...
/* Generic click-outside-aware dropdown trigger. Renders `trigger(open, setOpen)` as the
   button and `panel(setOpen)` as the floating content when open. */
function DropdownShell({ trigger, panel, panelClassName = "", align = "left" }) {
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      ...
      {open && (
        <div className={cx("vb-dropdown-panel", align === "right" ? "right-0" : "left-0", panelClassName)}>
          {panel(setOpen)}
        </div>
      )}
    </div>
...
/* "Chủ đề" filter dropdown — pick a topic (or "Tất cả chủ đề") to filter a list by. */
function TopicFilterDropdown({ topics, value, onChange, allLabel = "Tất cả chủ đề", counts, onDelete, requestConfirm }) {
  const handleDelete = (e, t) => {
    e.stopPropagation();
    if (!onDelete) return;
    if (requestConfirm) {
      requestConfirm(`Xoá chủ đề "${t}"? Các mục đang gắn chủ đề này sẽ được gỡ nhãn.`, () => onDelete(t), "Xoá chủ đề");
    } else {
      onDelete(t);
    }
  };
  return (
    <DropdownShell
      align="right"
      trigger={(open, setOpen) => (
        <button type="button" onClick={() => setOpen((v) => !v)} className={cx("vb-btn-secondary text-sm", value !== "all" && "vb-chip-toggle-active")}>
          <Tag size={15} /> {value === "all" ? "Chủ đề" : value} <ChevronDown size={13} />
        </button>
      )}
      panel={(setOpen) => (
        <div className="max-h-64 overflow-y-auto" style={{ minWidth: 220 }}>
          <button type="button" onClick={() => { onChange("all"); setOpen(false); }} className={cx("vb-dropdown-item", value === "all" && "vb-dropdown-item-active")}>
            <span>{allLabel}</span>
            {counts && <span className="text-xs opacity-80">{counts.all ?? ""}</span>}
          </button>
          {topics.length === 0 && <div className="text-xs px-3 py-2" style={{ color: "var(--vb-muted)" }}>Chưa có chủ đề nào</div>}
          {topics.map((t) => (
            <div key={t} className={cx("vb-dropdown-item", value === t && "vb-dropdown-item-active")} style={{ cursor: "pointer" }} onClick={() => { onChange(t); setOpen(false); }}>
              <span className="truncate">{t}</span>
              <span className="flex items-center gap-2 shrink-0">
                {counts && <span className="text-xs opacity-80">{counts[t] ?? ""}</span>}
                {onDelete && (
                  <button
                    type="button"
                    onClick={(e) => handleDelete(e, t)}
                    className="vb-topic-delete-btn"
                    title={`Xoá chủ đề "${t}"`}
                    aria-label={`Xoá chủ đề ${t}`}
                  >
                    <X size={12} />
                  </button>
                )}
...
/* "Gắn chủ đề" — pick an existing topic name or type a brand-new one, used to bulk-tag
   selected items. Calls onPick(topicName) once a choice is made; onCreate registers a
   brand-new custom name into the app's permanent topic list. */
function TopicAssignDropdown({ topics, onPick, onCreate, label = "Gắn chủ đề", disabled }) {
    ...
    <DropdownShell
      ...
      trigger={(open, setOpen) => (
        <button
          ...
          disabled={disabled}
          onClick={() => setOpen((v) => !v)}
          className="vb-btn-primary text-sm disabled:opacity-40"
          style={{ background: "var(--vb-honey)", borderColor: "var(--vb-honey)" }}
        >
          <Tag size={15} /> {label}
        </button>
      )}
      panel={(setOpen) => {
        ...
        return (
          <div style={{ minWidth: 220 }}>
            <div className="text-xs font-semibold uppercase tracking-wide px-2 pb-1.5" style={{ color: "var(--vb-muted)" }}>Chọn chủ đề có sẵn</div>
            <div className="max-h-44 overflow-y-auto mb-2">
              {topics.length === 0 && <div className="text-xs px-3 py-2" style={{ color: "var(--vb-muted)" }}>Chưa có chủ đề nào</div>}
              {topics.map((t) => (
                <button key={t} type="button" onClick={() => { onPick(t); setOpen(false); }} className="vb-dropdown-item">
                  <span className="truncate">{t}</span>
                </button>
              ))}
            </div>
            <div className="pt-2 flex gap-1.5" style={{ borderTop: "1px dashed var(--vb-border)" }}>
              <input
                value={customInput}
                onChange={(e) => setCustomInput(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") submitCustom(); }}
                placeholder="Tự đặt tên chủ đề mới..."
                className="vb-input flex-1 text-sm"
                autoFocus
              />
              <button type="button" onClick={submitCustom} disabled={!customInput.trim()} className="vb-btn-primary text-sm px-3 disabled:opacity-40"><Plus size={14} /></button>
            </div>
          </div>
        );
      }}
...
export default function App() {
  ...
  const [topics, setTopics] = useState(() => Array.from(new Set(
    withDemoProgress(seedVocab()).concat(seedIdioms(), seedCollocations()).map((i) => i.topic).filter(Boolean)
  )).sort());
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((_event, newSession) => {
      // Supabase silently refreshes the access token whenever the tab regains
      // focus (e.g. switching back from another tab or app), firing this
      // callback with a brand-new session object even though the signed-in
      // user hasn't actually changed. Naively calling setSession(newSession)
      // every time changes the `session` reference, which re-triggers the
      // "load persisted data" effect below and — because that effect flips on
      // the full-screen loading spinner while it fetches — unmounts whatever
      // flashcard/practice session was in progress. Only update state when the
      // signed-in user genuinely changes (login, logout, switching accounts).
      setSession((prev) => ((prev?.user?.id ?? null) === (newSession?.user?.id ?? null) ? prev : newSession));
    });
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      if (isSupabaseConfigured) {
        ...
        supabase
          .from("vocabloom_data")
          .upsert({ user_id: session.user.id, payload: { items, stats, earnedBadges, topics }, updated_at: new Date().toISOString() })
          .then(() => {})
          .catch(() => {});
        return;
      }
      try {
        localStorage.setItem("vocabloom-data", JSON.stringify({ items, stats, earnedBadges, topics }));
      } catch (e) { /* storage full or unavailable */ }
    }, 700);
    return () => clearTimeout(saveTimer.current);
  }, [items, stats, earnedBadges, topics, session]);
  ...
  const setItemsTopic = useCallback((ids, topic) => {
    const idSet = new Set(ids);
    const clean = String(topic || "").trim();
    if (!clean) return;
    setItems((prev) => prev.map((it) => (idSet.has(it.id) ? { ...it, topic: clean } : it)));
    setTopics((prev) => (prev.includes(clean) ? prev : [...prev, clean].sort()));
    showToast(`🏷️ Đã gắn chủ đề "${clean}" cho ${ids.length} mục.`);
  }, [showToast]);
  ...
  const deleteTopic = useCallback((topic) => {
    requestConfirm(
      `Xoá chủ đề "${topic}"? Các từ/thành ngữ/cụm từ đang gắn chủ đề này sẽ được gỡ nhãn, nhưng bản thân chúng vẫn giữ nguyên.`,
      () => {
        setTopics((prev) => prev.filter((t) => t !== topic));
        setItems((prev) => prev.map((it) => (it.topic === topic ? { ...it, topic: "" } : it)));
        showToast(`🗑️ Đã xoá chủ đề "${topic}".`);
      },
      "Xoá chủ đề"
    );
  }, [showToast]);
  ...
  return (
    <div className={cx("vb-root", `vb-theme-${theme}`, "min-h-screen w-full flex flex-col lg:flex-row")}>
      ...
      <main className="flex-1 min-w-0 px-4 py-6 sm:px-8 sm:py-8 lg:px-10 lg:py-9 max-w-6xl mx-auto w-full">
        ...
        {activeTab === "myvocab" && (
          <MyVocabulary items={items} toggleFavorite={toggleFavorite} openDetail={setDetailItem} addManualItem={addManualItem} deleteItem={deleteItem} deleteItems={deleteItems} requestConfirm={requestConfirm} setItemGrade={setItemGrade} setItemStatus={setItemStatus} setItemsTopic={setItemsTopic} topics={topics} addTopic={addTopic} deleteTopic={deleteTopic} />
        )}
        {activeTab === "flashcards" && (
          <FlashcardsPage items={items} gradeItem={gradeItem} showToast={showToast} recordSentence={recordSentence} topics={topics} />
        )}
        {activeTab === "practice" && (
          <PracticeView items={items} gradeItem={gradeItem} recordSentence={recordSentence} quickPool={quickPool} clearQuickPool={() => setQuickPool(null)} goto={goto} topics={topics} />
        )}
        {activeTab === "idioms" && (
          <IdiomsCollocationsList type="idiom" items={items} toggleFavorite={toggleFavorite} openDetail={setDetailItem} showToast={showToast} addManualItem={addManualItem} deleteItems={deleteItems} requestConfirm={requestConfirm} setItemsTopic={setItemsTopic} topics={topics} addTopic={addTopic} deleteTopic={deleteTopic} />
        )}
        {activeTab === "collocations" && (
          <IdiomsCollocationsList type="collocation" items={items} toggleFavorite={toggleFavorite} openDetail={setDetailItem} showToast={showToast} addManualItem={addManualItem} deleteItems={deleteItems} requestConfirm={requestConfirm} setItemsTopic={setItemsTopic} topics={topics} addTopic={addTopic} deleteTopic={deleteTopic} />
        )}
        {activeTab === "statistics" && (
          <StatisticsView items={items} stats={stats} earnedBadges={earnedBadges} startWeakPractice={startWeakPractice} />
        )}
        {activeTab === "import" && (
          <ImportView items={items} setItems={setItems} showToast={showToast} goto={goto} topics={topics} addTopic={addTopic} />
        )}
...
const ROAST_QUOTES = [
  ...
  (s) => `${s.vocabTotal} từ nằm im re trong kho, tụi nó buồn lắm đó nha 🥲`,
  (s) => s.newCount > 0
    ? `Còn nguyên ${s.newCount} từ chưa đụng tới, học ít thôi mà lười cũng nhiều à nha 😂`
    : "Học ít thôi mà lười cũng nhiều, vậy chữ nào cũng không chịu ở lại đâu 😂",
  (s) => `Streak ${s.streak} ngày rồi đó, mà đứt là tự nhiên thấy cuộc đời vô nghĩa liền á, giữ lấy đi!`,
  () => "Đừng nói \"để mai học\", vì \"mai\" của bạn nó dài như deadline vậy đó 📅",
  (s) => s.newCount > 0
    ? `${s.newCount} từ chưa graded lần nào hết vậy? Để mồ côi từ vựng luôn hả 😩`
    : "Từ nào cũng graded hết rồi hen, giỏi dữ, đừng dừng lại nha 😩",
  () => "Học 5 phút thôi cũng được, hơn là ngồi đây đọc mấy câu mắng vui của tui nè 😜",
  (s) => s.weakCount > 0
    ? `${s.weakCount} từ đang sai lên sai xuống kìa, nút "Ôn lại lỗi sai" nó gọi tên bạn suốt đó, nghe chưa? 📢`
    : "Chưa sai từ nào để ôn lại luôn, xạo đó nha, đi học tiếp đi 📢",
];
...
function Dashboard({ items, stats, stat, goto, openDetail, startWeakPractice }) {
  ...
  const roastStats = useMemo(() => ({
    vocabTotal: stat.vocabTotal,
    streak: stats.streak,
    newCount: items.filter((i) => i.status === "new").length,
    weakCount: items.filter((i) => (i.reviews?.incorrect || 0) > 0).length,
  }), [items, stat.vocabTotal, stats.streak]);

  // Random on every visit to the dashboard (not tied to the date), so the
  // teasing line feels fresh each time someone opens the app.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const vibe = useMemo(() => pickRandom(VIBE_QUOTES), []);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const roast = useMemo(() => pickRandom(ROAST_QUOTES)(roastStats), []);
...
function MyVocabulary({ items, toggleFavorite, openDetail, addManualItem, deleteItem, deleteItems, requestConfirm, setItemGrade, setItemStatus, setItemsTopic, topics: allTopics, addTopic, deleteTopic }) {
  ...
  const vocab = items.filter((i) => i.type === "vocab");
  const topics = allTopics && allTopics.length ? allTopics : Array.from(new Set(vocab.map((v) => v.topic).filter(Boolean))).sort();
  ...
  const handleAssignTopic = (t) => { setItemsTopic(selected, t); exitSelectMode(); };
  const handleDeleteTopic = (t) => { if (topic === t) setTopic("all"); deleteTopic(t); };
  ...
  return (
    <div>
      ...
      {selectMode && (
        <div className="vb-bulk-bar mb-4 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-2">
            <TopicAssignDropdown topics={topics} onPick={handleAssignTopic} onCreate={addTopic} disabled={selected.length === 0} />
            <button onClick={handleBulkDelete} disabled={selected.length === 0} className="vb-btn-primary text-sm disabled:opacity-40" style={{ background: "#B4563F", borderColor: "#B4563F" }}>
      ...
      <div className="vb-card rounded-2xl p-4 mb-5 flex flex-wrap gap-3 items-center">
        ...
        <TopicFilterDropdown topics={topics} value={topic} onChange={setTopic} onDelete={handleDeleteTopic} allLabel="Mọi chủ đề" />
      ...
      {showAdd && <AddItemModal initialType="vocab" onAdd={addManualItem} onClose={() => setShowAdd(false)} topics={topics} onCreateTopic={addTopic} />}
    </div>
  );
}
...
function AddItemModal({ initialType = "vocab", onAdd, onClose, topics = [], onCreateTopic }) {
  ...
  const submit = () => {
    if (!canSubmit) return;
    const cleanTopic = topic.trim() || "Từ của tôi";
    const fields = {
      term: term.trim(),
    ...
    if (onCreateTopic) onCreateTopic(cleanTopic);
    onAdd(type, fields);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6" style={{ background: "rgba(30,45,35,0.4)" }} onClick={onClose}>
      <div className="vb-card w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 sm:p-7">
          ...
          <input value={topic} onChange={(e) => setTopic(e.target.value)} className="vb-input w-full mb-6" placeholder="Từ của tôi" list="vb-topic-suggestions" />
          {topics.length > 0 && (
            <datalist id="vb-topic-suggestions">
...
const FLASHCARD_TYPE_FILTERS = [
  { id: "all", label: "Tất cả (Từ vựng + Thành ngữ + Cụm từ)", shortLabel: "Tất cả" },
  { id: "vocab", label: "Từ vựng", shortLabel: "Từ vựng" },
  { id: "idiom", label: "Thành ngữ", shortLabel: "Thành ngữ" },
  { id: "collocation", label: "Cụm từ đi kèm", shortLabel: "Cụm từ đi kèm" },
];

const FLASHCARD_STATUS_FILTERS = [
  { id: "all", label: "Tất cả" },
  { id: "new", label: "Từ mới" },
...
function poolForFlashcardFilters(items, { typeFilter, statusFilter, topicFilter }) {
  let pool = typeFilter === "all" ? items : items.filter((i) => i.type === typeFilter);
  if (topicFilter && topicFilter !== "all") pool = pool.filter((i) => i.topic === topicFilter);
  switch (statusFilter) {
    case "new": return pool.filter((i) => i.status === "new");
    case "learned": return pool.filter((i) => i.status !== "new");
    case "favorite": return pool.filter((i) => i.favorite);
    case "grade-again": return pool.filter((i) => i.grade === "again");
    case "grade-hard": return pool.filter((i) => i.grade === "hard");
    case "grade-good": return pool.filter((i) => i.grade === "good");
    case "grade-easy": return pool.filter((i) => i.grade === "easy");
    default: return pool;
  }
}

function FlashcardsPage({ items, gradeItem, showToast, recordSentence, topics: globalTopics }) {
  const [typeFilter, setTypeFilter] = useState("all");
  const [statusFilter, setStatusFilter] = useState("all");
  const [topicFilter, setTopicFilter] = useState("all");
  const [activeIds, setActiveIds] = useResumableState("vb-active-flashcards", null); // null | [id, id, ...]

  const scopeForTopics = typeFilter === "all" ? items : items.filter((i) => i.type === typeFilter);
  const topics = (globalTopics && globalTopics.length ? globalTopics : Array.from(new Set(items.map((i) => i.topic).filter(Boolean)))).filter((t) => scopeForTopics.some((i) => i.topic === t));
  // Topic filter may reference a topic that doesn't exist once the type scope changes; fall back gracefully.
  const effectiveTopicFilter = topics.includes(topicFilter) ? topicFilter : "all";

  const pool = poolForFlashcardFilters(items, { typeFilter, statusFilter, topicFilter: effectiveTopicFilter });

  if (activeIds) {
    // Re-resolve against the live items list (not a frozen snapshot) so grading
    // progress made just before a reload is reflected, then hand off to the
    // deck runner — which itself resumes mid-deck via sessionStorage. This is
    // what lets switching tabs/apps and coming back pick up right where you left off.
    const resumedPool = items.filter((i) => activeIds.includes(i.id));
    return (
      <FlashcardDeckRunner
        pool={resumedPool}
        gradeItem={gradeItem}
        recordSentence={recordSentence}
        sessionKey="flashcards"
        onExit={() => setActiveIds(null)}
        onFinish={() => { setActiveIds(null); showToast("Xong bộ thẻ — làm tốt lắm! 🌱"); }}
      />
    );
  }
    ...
    <div>
      ...
      <div className="vb-card vb-pop rounded-3xl p-6 max-w-xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>Loại</label>
            <select className="vb-select w-full" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}>
              {FLASHCARD_TYPE_FILTERS.map((t) => (
                <option key={t.id} value={t.id}>{t.label} ({poolForFlashcardFilters(items, { typeFilter: t.id, statusFilter: "all", topicFilter: "all" }).length})</option>
              ))}
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>Trạng thái / Điểm</label>
            <select className="vb-select w-full" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
              {FLASHCARD_STATUS_FILTERS.map((f) => (
                <option key={f.id} value={f.id}>{f.label} ({poolForFlashcardFilters(items, { typeFilter, statusFilter: f.id, topicFilter: effectiveTopicFilter }).length})</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div className="mt-3">
          ...
          <TopicFilterDropdown
            ...
            counts={{
              all: poolForFlashcardFilters(items, { typeFilter, statusFilter, topicFilter: "all" }).length,
              ...Object.fromEntries(topics.map((t) => [t, poolForFlashcardFilters(items, { typeFilter, statusFilter, topicFilter: t }).length])),
            }}
          />
        </div>

        <div className="mt-5 pt-5 flex items-center justify-between" style={{ borderTop: "2px dashed var(--vb-border)" }}>
          <span className="text-sm font-semibold" style={{ color: "var(--vb-sage-dk)" }}>Bộ thẻ hiện tại: {pool.length} thẻ</span>
        </div>

        <button
          disabled={pool.length === 0}
          onClick={() => setActiveIds(pool.map((i) => i.id))}
          className="vb-btn-primary w-full justify-center mt-4 disabled:opacity-40"
        >
...
function FlashcardDeckRunner({ pool, gradeItem, recordSentence, onExit, onFinish, sessionKey = "flashcards" }) {
  const runnerStorageKey = `vb-runner-${sessionKey}`;
  const [byId] = useState(() => Object.fromEntries(pool.map((i) => [i.id, i])));
  const poolIds = useState(() => pool.map((i) => i.id))[0];

  // Resume mid-deck progress after an accidental reload (mobile tab discard, a
  // background token refresh, etc.) as long as the saved deck still matches
  // this exact set of cards; otherwise start a fresh shuffled deck.
  const [rs, setRs] = useState(() => {
    const saved = readSessionSlot(runnerStorageKey);
    const savedDeckValid = saved && Array.isArray(saved.deck) && saved.deck.length === poolIds.length && saved.deck.every((id) => poolIds.includes(id));
    if (savedDeckValid) {
      return { deck: saved.deck, queue: saved.queue, remaining: saved.remaining, showBack: !!saved.showBack, sentenceValue: saved.sentenceValue || "" };
    }
    const freshDeck = shuffle(poolIds);
    return { deck: freshDeck, queue: freshDeck, remaining: freshDeck, showBack: false, sentenceValue: "" };
  });
  const { deck, queue, remaining, showBack, sentenceValue } = rs;

  useEffect(() => { writeSessionSlot(runnerStorageKey, rs); }, [runnerStorageKey, rs]);

  const exit = () => { clearSessionSlot(runnerStorageKey); onExit(); };
  const finish = () => { clearSessionSlot(runnerStorageKey); onFinish(); };

  if (deck.length === 0) {
    return <EmptyState icon={Layers} title="Chưa có thẻ nào" subtitle="Thử chọn bộ thẻ khác xem sao." action={<button onClick={exit} className="vb-btn-secondary mt-4">Quay lại</button>} />;
  }
    ...
    return (
      <div className="vb-card rounded-3xl p-8 max-w-md mx-auto text-center">
        ...
        <div className="flex gap-2.5 justify-center">
          <button onClick={finish} className="vb-btn-primary"><Home size={15} /> Về trang chủ</button>
        </div>
      </div>
    );
  }
  ...
  const done = deck.length - remaining.length;
  const canWriteSentence = recordSentence && (card.type === "idiom" || card.type === "collocation");

  const grade = (g) => {
    const trimmed = sentenceValue.trim();
    if (canWriteSentence && trimmed && recordSentence) {
      recordSentence(currentId, trimmed, sentenceUsesTerm(trimmed, card.term));
    }
    ...
    if (g === "again") {
      ...
      nRemaining = remaining.filter((id) => id !== currentId);
    }
    setRs({ deck, queue: nq, remaining: nRemaining, showBack: false, sentenceValue: "" });
  };
  const setShowBack = (v) => setRs((prev) => ({ ...prev, showBack: v }));
  const setSentenceValue = (v) => setRs((prev) => ({ ...prev, sentenceValue: v }));

  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <button onClick={exit} className="text-sm font-semibold flex items-center gap-1" style={{ color: "var(--vb-sage-dk)" }}><ChevronLeft size={16} /> Thoát</button>
        ...
        <div className="text-center flex-1 flex flex-col items-center justify-center w-full">
          ...
          {showBack && (
            <div className="mt-5 pt-5 w-full text-left" style={{ borderTop: "2px dashed var(--vb-border)" }}>
              ...
              {canWriteSentence && (
                <div className="mt-4">
                  <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>
                    Tự đặt câu với "{card.term}" (không bắt buộc)
                  </label>
                  <textarea
                    value={sentenceValue}
                    onChange={(e) => setSentenceValue(e.target.value)}
                    className="vb-input w-full resize-none text-sm"
                    rows={2}
                    placeholder="Viết một câu tiếng Anh dùng cụm này... hoặc bỏ trống rồi qua thẻ tiếp theo."
                  />
                  <p className="text-xs mt-1" style={{ color: "var(--vb-muted)" }}>Không đặt câu vẫn có thể chấm điểm và qua thẻ tiếp theo bình thường.</p>
                </div>
              )}
            </div>
          )}
...
function PracticeView({ items, gradeItem, recordSentence, quickPool, clearQuickPool, goto, topics: globalTopics }) {
  const [typeFilter, setTypeFilter] = useState("all");
  const [statusFilter, setStatusFilter] = useState("all");
  const [topicFilter, setTopicFilter] = useState("all");
  const [questionType, setQuestionType] = useState("e2v");
  const [count, setCount] = useState(10);
  const [customMode, setCustomMode] = useState(false);
  const [customSelection, setCustomSelection] = useState([]);
  const [pickerSearch, setPickerSearch] = useState("");
  const [session, setSession] = useResumableState("vb-active-practice", null); // null | { ids: [...], questionType }

  useEffect(() => {
    if (quickPool) {
      setSession({ ids: quickPool, questionType: "e2v" });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [quickPool]);

  if (session) {
    // Re-resolve ids against live items (not a frozen snapshot) on every render,
    // and let each sub-runner resume its own mid-question progress from
    // sessionStorage — so switching tabs/apps mid-practice no longer forces you
    // to start the set over.
    const pool = items.filter((i) => session.ids.includes(i.id));
    return (
      <PracticeRunner
        pool={pool}
  ...
  const buildPool = () => {
    if (customMode) return items.filter((i) => customSelection.includes(i.id));
    return poolForFlashcardFilters(items, { typeFilter, statusFilter, topicFilter: effectiveTopicFilter });
  };

  const start = () => {
    const pool = customMode ? buildPool() : shuffle(buildPool()).slice(0, count);
    if (pool.length === 0) return;
    setSession({ ids: pool.map((i) => i.id), questionType });
  };

  const poolSize = buildPool().length;
  const pickerScope = topicFilter === "all" ? scopeForTopics : scopeForTopics.filter((i) => i.topic === effectiveTopicFilter);
  const pickerList = pickerScope.filter((i) => `${i.term} ${i.meaning}`.toLowerCase().includes(pickerSearch.toLowerCase()));
  ...
  return (
    <div>
      ...
      <div className="vb-card vb-pop rounded-3xl p-6 max-w-xl mx-auto">
        <div className="text-sm font-semibold mb-3" style={{ color: "var(--vb-ink)" }}>Bạn muốn luyện gì nào? 🌟</div>

        <div className="grid sm:grid-cols-2 gap-3 mb-1">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>Loại</label>
            <select
              className="vb-select w-full"
              value={customMode ? "custom" : typeFilter}
              onChange={(e) => {
            ...
            >
              {FLASHCARD_TYPE_FILTERS.map((t) => <option key={t.id} value={t.id}>{t.label}</option>)}
              <option value="custom">Chọn từ cụ thể</option>
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>Trạng thái / Điểm</label>
            <select className="vb-select w-full" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} disabled={customMode}>
              {FLASHCARD_STATUS_FILTERS.map((f) => <option key={f.id} value={f.id}>{f.label}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="mt-3 mb-5">
          <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>Chủ đề</label>
          <TopicFilterDropdown topics={topics} value={effectiveTopicFilter} onChange={setTopicFilter} />
        </div>
        ...
        ]} value={questionType} onChange={setQuestionType} />

        {customMode ? (
          <div className="mb-5">
            <div className="text-sm font-semibold mb-2" style={{ color: "var(--vb-ink)" }}>Chọn những từ muốn ôn — trình bày như thẻ ghi nhớ</div>
            ...
            <div className="grid grid-cols-2 gap-2.5 overflow-y-auto pr-1" style={{ maxHeight: 320 }}>
              {pickerList.length === 0 ? (
                <p className="text-sm text-center py-6 col-span-2" style={{ color: "var(--vb-muted)" }}>Không tìm thấy từ nào.</p>
              ) : pickerList.map((it) => {
                const active = customSelection.includes(it.id);
                return (
                  <button
                    type="button"
                    key={it.id}
                    onClick={() => toggleCustom(it.id)}
                    className="vb-pick-card text-left"
                    style={active ? { borderColor: "var(--vb-mint-dk)", background: "var(--vb-mint-pale)" } : undefined}
                  >
                    <div className="flex items-start justify-between gap-1.5">
                      <span className="text-xs font-medium uppercase tracking-wide" style={{ color: "var(--vb-muted)" }}>{typeLabel(it.type)}</span>
                      {active && <CheckCircle2 size={15} color="var(--vb-mint-dk)" />}
                    </div>
                    <div className="font-semibold text-sm mt-1 truncate" style={{ color: "var(--vb-ink)" }}>{it.term}</div>
                    <div className="text-xs mt-0.5 truncate" style={{ color: "var(--vb-muted)" }}>{it.meaning}</div>
                  </button>
                );
              })}
            </div>
          ...
          <PracticeConfigGroup label="Số lượng câu hỏi" options={[10, 20, 30, 50].map((n) => ({ id: n, label: String(n) }))} value={count} onChange={setCount} />
        )}
        ...
        <p className="text-xs text-center mt-2" style={{ color: "var(--vb-muted)" }}>
          {customMode
            ...
            : `Có ${poolSize} mục trong kho từ của bạn khớp với lựa chọn này${poolSize < count ? " — buổi luyện chỉ dùng số từ đang có, không lấy thêm ngoài kho của bạn" : ""}.`}
        </p>
      </div>
    </div>
  );
...
function PracticeRunner({ pool, questionType, gradeItem, recordSentence, onExit }) {
  if (questionType === "flashcard") {
    return <FlashcardDeckRunner pool={pool} gradeItem={gradeItem} recordSentence={recordSentence} sessionKey="practice-flashcard" onExit={onExit} onFinish={onExit} />;
  }
...
function MCQuizRunner({ pool, direction, gradeItem, onExit, sessionKey = "practice-mc" }) {
  const runnerStorageKey = `vb-runner-${sessionKey}`;
  const poolIds = useState(() => pool.map((i) => i.id))[0];

  const makeFreshState = () => {
    const questions = shuffle(pool).map((it) => makeMCQuestion(it, pool, direction));
    return {
      questions,
      questionItemIds: questions.map((q) => q.item.id),
      direction,
      idx: 0, selected: null, correctCount: 0, mistakes: [], finishedMs: null, restartKey: 0, startedAt: Date.now(),
    };
  };

  const [rs, setRs] = useState(() => {
    const saved = readSessionSlot(runnerStorageKey);
    const savedValid = saved && Array.isArray(saved.questionItemIds) && saved.direction === direction
      && saved.questionItemIds.length === poolIds.length && saved.questionItemIds.every((id) => poolIds.includes(id));
    return savedValid ? saved : makeFreshState();
  });
  useEffect(() => { writeSessionSlot(runnerStorageKey, rs); }, [runnerStorageKey, rs]);
  const { questions, idx, selected, correctCount, mistakes, finishedMs, restartKey, startedAt } = rs;
  const elapsed = () => Date.now() - startedAt;

  const exit = () => { clearSessionSlot(runnerStorageKey); onExit(); };

  if (questions.length === 0) {
    return <EmptyState icon={Target} title="Chưa đủ từ để luyện" subtitle="Thêm từ vựng để tạo bài luyện tập." action={<button onClick={exit} className="vb-btn-secondary mt-4">Quay lại</button>} />;
  }
  ...
  if (finishedMs !== null) {
    return (
      <PracticeResult
        ...
        onAgain={() => setRs({ ...makeFreshState(), restartKey: restartKey + 1 })}
        onReviewMistakes={exit} onDone={exit}
      />
    );
  }
  ...
  const choose = (opt) => {
    ...
    setRs((prev) => ({
      ...
      mistakes: isCorrect ? prev.mistakes : [...prev.mistakes, q.item],
    }));
  };

  const next = () => {
    if (idx + 1 >= questions.length) { setRs((prev) => ({ ...prev, finishedMs: elapsed() })); return; }
    setRs((prev) => ({ ...prev, idx: prev.idx + 1, selected: null }));
  };
...
function TypingRunner({ pool, gradeItem, onExit, sessionKey = "practice-typing" }) {
  const runnerStorageKey = `vb-runner-${sessionKey}`;
  const poolIds = useState(() => pool.map((i) => i.id))[0];
  const byId = useState(() => Object.fromEntries(pool.map((i) => [i.id, i])))[0];

  const makeFreshState = () => {
    const questionIds = shuffle(poolIds);
    return { questionIds, idx: 0, value: "", checked: null, correctCount: 0, mistakeIds: [], finishedMs: null, restartKey: 0, startedAt: Date.now() };
  };

  const [rs, setRs] = useState(() => {
    const saved = readSessionSlot(runnerStorageKey);
    const savedValid = saved && Array.isArray(saved.questionIds) && saved.questionIds.length === poolIds.length && saved.questionIds.every((id) => poolIds.includes(id));
    return savedValid ? saved : makeFreshState();
  });
  useEffect(() => { writeSessionSlot(runnerStorageKey, rs); }, [runnerStorageKey, rs]);
  const { questionIds, idx, value, checked, correctCount, mistakeIds, finishedMs, restartKey, startedAt } = rs;
  const questions = questionIds.map((id) => byId[id]);
  const mistakes = mistakeIds.map((id) => byId[id]);
  const elapsed = () => Date.now() - startedAt;

  const exit = () => { clearSessionSlot(runnerStorageKey); onExit(); };

  if (questions.length === 0) {
    return <EmptyState icon={PenLine} title="Chưa đủ từ để luyện" subtitle="Thêm từ vựng để luyện gõ chữ." action={<button onClick={exit} className="vb-btn-secondary mt-4">Quay lại</button>} />;
  }
  ...
  const check = () => {
    ...
    setRs((prev) => ({
      ...prev,
      checked: isCorrect ? "correct" : "incorrect",
      correctCount: isCorrect ? prev.correctCount + 1 : prev.correctCount,
      mistakeIds: isCorrect ? prev.mistakeIds : [...prev.mistakeIds, q.id],
    }));
  };

  const next = () => {
    if (idx + 1 >= questions.length) { setRs((prev) => ({ ...prev, finishedMs: elapsed() })); return; }
    setRs((prev) => ({ ...prev, idx: prev.idx + 1, value: "", checked: null }));
  };
  ...
  return (
    <div key={restartKey}>
      ...
      <div className="vb-card rounded-3xl p-7 max-w-lg mx-auto">
        ...
        <input
          value={value}
          onChange={(e) => setRs((prev) => ({ ...prev, value: e.target.value }))}
...
function SentenceRunner({ pool, gradeItem, recordSentence, onExit, sessionKey = "practice-sentence" }) {
  const runnerStorageKey = `vb-runner-${sessionKey}`;
  const poolIds = useState(() => pool.map((i) => i.id))[0];
  const byId = useState(() => Object.fromEntries(pool.map((i) => [i.id, i])))[0];
  ...
  if (questions.length === 0) {
    return <EmptyState icon={PenLine} title="Chưa đủ từ để luyện" subtitle="Thêm từ vựng trước đã nhé." action={<button onClick={exit} className="vb-btn-secondary mt-4">Quay lại</button>} />;
  }
  ...
  const check = () => {
    ...
    setRs((prev) => ({
      ...prev,
      checked: { ok, short: wc < 4 },
      correctCount: ok ? prev.correctCount + 1 : prev.correctCount,
      mistakeIds: ok ? prev.mistakeIds : [...prev.mistakeIds, q.id],
    }));
  };
...
function IdiomsCollocationsList({ type, items, toggleFavorite, openDetail, showToast, addManualItem, deleteItems, requestConfirm, setItemsTopic, topics: allTopics, addTopic, deleteTopic }) {
  const [q, setQ] = useState("");
  const [showAdd, setShowAdd] = useState(false);
  const [selectMode, setSelectMode] = useState(false);
  const [selected, setSelected] = useState([]);
  const [topicFilter, setTopicFilter] = useState("all");
  const allOfType = items.filter((i) => i.type === type);
  const topics = allTopics && allTopics.length ? allTopics : Array.from(new Set(allOfType.map((i) => i.topic).filter(Boolean))).sort();
  const list = allOfType.filter((i) => `${i.term} ${i.meaning}`.toLowerCase().includes(q.toLowerCase()) && (topicFilter === "all" || i.topic === topicFilter));
  ...
  const handleAssignTopic = (topic) => { setItemsTopic(selected, topic); exitSelectMode(); };
  const handleDeleteTopic = (topic) => { if (topicFilter === topic) setTopicFilter("all"); deleteTopic(topic); };

  return (
    <div>
      <SectionHeader title={title} subtitle={`${allOfType.length} ${title.toLowerCase()} để khám phá`} right={
        <div className="flex items-center gap-2 flex-wrap justify-end">
          <div className="relative">
            <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2" color="var(--vb-muted)" />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm kiếm..." className="vb-input pl-9" />
          </div>
          <TopicFilterDropdown topics={topics} value={topicFilter} onChange={setTopicFilter} onDelete={handleDeleteTopic} />
          <button onClick={() => (selectMode ? exitSelectMode() : setSelectMode(true))} className={cx("vb-btn-secondary text-sm", selectMode && "vb-chip-toggle-active")}>
            <ListChecks size={15} /> {selectMode ? "Xong" : "Chọn nhiều"}
          </button>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {list.map((it) => (
            <div key={it.id} className="vb-card rounded-2xl p-5 cursor-pointer relative" onClick={() => (selectMode ? toggleSelected(it.id) : openDetail(it))}>
              ...
              {it.topic && (
                <span className="vb-chip text-xs px-2 py-0.5 inline-flex items-center gap-1 mt-1.5" style={{ fontWeight: 600 }}>
                  <Tag size={10} /> {it.topic}
                </span>
              )}
      ...
      {showAdd && <AddItemModal initialType={type} onAdd={addManualItem} onClose={() => setShowAdd(false)} topics={topics} onCreateTopic={addTopic} />}
    </div>
  );
}
...
function StatisticsView({ items, stats, earnedBadges, startWeakPractice }) {
  ...
  // Real cumulative review count built from the user's actual daily activity
  // log (stats.dailyActivity), instead of a fabricated straight-line ramp.
  const progressData = (() => {
    let cumulative = 0;
    const out = [];
    for (let i = 6; i >= 0; i--) {
      ...
      const key = fmtDate(d);
      cumulative += stats.dailyActivity[key] || 0;
      out.push({
        date: i === 0 ? "Hôm nay" : i === 1 ? "Hôm qua" : `${i} ngày trước`,
        words: cumulative,
      });
    }
    return out;
  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-5 mb-6">
        <ChartCard title="Tiến độ học" subtitle="Lượt ôn cộng dồn trong tuần qua">
...
function ImportView({ items, setItems, showToast, goto, topics: globalTopics, addTopic }) {
  ...
  const existingTopics = globalTopics && globalTopics.length ? globalTopics : Array.from(new Set(items.map((i) => i.topic).filter(Boolean))).sort();
  ...
  const confirmImport = () => {
    ...
    rows.forEach((r) => {
      ...
      toAdd.push(makeItem(cfg.itemType, { ...fields, topic: batchTopic.trim() || fields.topic || "Đã nhập", source: "import" }));
    });
    ...
    showToast(`✅ Đã nhập thành công ${toAdd.length} mục ${cfg.label.toLowerCase()}!${skipped ? ` (bỏ qua ${skipped} mục trùng)` : ""}${batchTopic.trim() ? ` — chủ đề "${batchTopic.trim()}"` : ""}`);
    if (addTopic) {
      const topicsUsed = new Set(toAdd.map((it) => it.topic).filter(Boolean));
      topicsUsed.forEach((t) => addTopic(t));
    }
    setPreview(null);
    setBatchTopic("");
    if (fileRef.current) fileRef.current.value = "";
              ...
              <DropdownShell
                ...
                trigger={(open, setOpen) => (
                  <button
                    type="button"
                    onClick={() => setOpen((v) => !v)}
                    className={cx("vb-btn-secondary text-sm", batchTopic.trim() && "vb-chip-toggle-active")}
                  >
                    <Tag size={15} /> {batchTopic.trim() ? `Chủ đề: ${batchTopic.trim()}` : "Nhập chủ đề"} <ChevronDown size={13} />
                  </button>
                )}
                panel={(setOpen) => (
                  <div style={{ minWidth: 240 }}>
                    <div className="text-xs font-semibold uppercase tracking-wide px-2 pb-1.5" style={{ color: "var(--vb-muted)" }}>Gắn cho toàn bộ lô này</div>
                    {existingTopics.length > 0 && (
                      <div className="max-h-36 overflow-y-auto mb-2">
                        {existingTopics.map((t) => (
                          <button key={t} type="button" onClick={() => { setBatchTopic(t); setOpen(false); }} className={cx("vb-dropdown-item", batchTopic === t && "vb-dropdown-item-active")}>
                            <span className="truncate">{t}</span>
                          </button>
                        ))}
                      </div>
                    )}
                    <div className="pt-2 flex gap-1.5" style={{ borderTop: existingTopics.length ? "1px dashed var(--vb-border)" : "none" }}>
                      <input
                        value={batchTopic}
                        onChange={(e) => setBatchTopic(e.target.value)}
                        onKeyDown={(e) => { if (e.key === "Enter") setOpen(false); }}
                        placeholder="Tự đặt tên chủ đề..."
                        className="vb-input flex-1 text-sm"
                        autoFocus
                      />
                      {batchTopic.trim() && (
                        <button type="button" onClick={() => setBatchTopic("")} className="vb-icon-btn" title="Bỏ chủ đề"><X size={15} /></button>
                      )}
                    </div>
                    <p className="text-xs mt-2 px-1" style={{ color: "var(--vb-muted)" }}>Để trống thì dùng chủ đề trong file (nếu có), hoặc "Đã nhập".</p>
                  </div>
                )}
              />
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "var(--vb-mint-pale)" }}>
                  ...
                  <th className="text-left px-4 py-2.5 font-semibold" style={{ color: "var(--vb-sage-dk)" }}>Chủ đề</th>
                ...
                {preview.rows.slice(0, 50).map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-4 py-2.5">
                      <span className="vb-chip text-xs px-2 py-0.5">{batchTopic.trim() || r.topic || "Đã nhập"}</span>
                    </td>
...
.vb-pick-card { display: block; width: 100%; padding: 10px 12px; border-radius: 16px; border: 2px solid var(--vb-border); background: white; transition: transform 0.12s ease, border-color 0.12s ease; }
.vb-pick-card:hover { transform: translateY(-1px); border-color: var(--vb-mint-dk); }
...
.vb-dropdown-panel { position: absolute; z-index: 40; margin-top: 8px; background: var(--vb-card-bg); border: 2px solid var(--vb-border); border-radius: 18px; box-shadow: 0 6px 0 var(--vb-border), 0 12px 26px rgba(34,53,38,0.14); padding: 8px; }
.vb-dropdown-item { width: 100%; text-align: left; padding: 9px 12px; border-radius: 12px; font-size: 13px; font-weight: 600; color: var(--vb-ink); display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.vb-dropdown-item:hover { background: var(--vb-mint-pale); }
.vb-dropdown-item-active { background: var(--vb-mint-dk); color: white; }
.vb-dropdown-item-active:hover { background: var(--vb-mint-dk); }
.vb-topic-delete-btn { display: inline-flex; align-items: center; justify-content: center; width: 20px; height: 20px; border-radius: 999px; color: inherit; opacity: 0.55; }
.vb-topic-delete-btn:hover { opacity: 1; background: rgba(180,86,63,0.18); color: #B4563F; }
