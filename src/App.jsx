...
const TEMPLATE_HEADER = ["Vocabulary", "Part of Speech", "Vietnamese Meaning", "English Example"];
...
function withDemoProgress(vocab) {
  ...
  set("curious", { status: "mastered", box: 4, reviews: { correct: 6, incorrect: 0 }, lastReviewedAt: Date.now() - 1000 * 60 * 60 * 5 });
  set("genuine", { status: "mastered", box: 5, reviews: { correct: 5, incorrect: 1 }, lastReviewedAt: Date.now() - 1000 * 60 * 60 * 26 });
  set("diligent", { status: "familiar", box: 2, reviews: { correct: 3, incorrect: 1 }, lastReviewedAt: Date.now() - 1000 * 60 * 60 * 30 });
  set("achieve", { status: "familiar", box: 2, reviews: { correct: 2, incorrect: 0 }, lastReviewedAt: Date.now() - 1000 * 60 * 60 * 50 });
  set("ambiguous", { status: "learning", box: 1, reviews: { correct: 2, incorrect: 3 }, lastReviewedAt: Date.now() - 1000 * 60 * 60 * 10 });
  set("inevitable", { status: "learning", box: 1, reviews: { correct: 1, incorrect: 2 }, lastReviewedAt: Date.now() - 1000 * 60 * 60 * 12 });
  set("flourish", { status: "learning", box: 0, reviews: { correct: 1, incorrect: 2 }, lastReviewedAt: Date.now() - 1000 * 60 * 60 * 20 });
...
function DifficultyPill({ level }) {
  const map = { easy: "#3E7A6A", medium: "#C08A34", hard: "#B4563F" };
  const bg = { easy: "#E1F1EC", medium: "#FBF1DC", hard: "#F7E3DD" };
  return (
    <span className="text-xs font-medium px-2.5 py-1 rounded-full capitalize" style={{ color: map[level] || map.medium, background: bg[level] || bg.medium }}>
      {level}
    </span>
  );
}
...
export default function App() {
  ...
  const gradeItem = useCallback((id, grade) => {
    setItems((prev) => prev.map((it) => {
      ...
      if (grade === "again") { box = 0; reviews.incorrect += 1; correctFlag = false; }
      ...
      return { ...it, box, reviews, status, lastReviewedAt: Date.now() };
    }));
  ...
  const addManualWord = useCallback((fields) => {
    const item = makeItem("vocab", { ...fields, source: "manual" });
    setItems((prev) => [...prev, item]);
    showToast(`✅ Added "${item.term}" to your vocabulary.`);
  }, [showToast]);
  ...
  return (
    <div className="vb-root min-h-screen w-full flex flex-col lg:flex-row">
      ...
      <main className="flex-1 min-w-0 px-4 py-6 sm:px-8 sm:py-8 lg:px-10 lg:py-9 max-w-6xl mx-auto w-full">
        ...
        {activeTab === "myvocab" && (
          <MyVocabulary items={items} toggleFavorite={toggleFavorite} openDetail={setDetailItem} addManualWord={addManualWord} deleteItem={deleteItem} requestConfirm={requestConfirm} />
        )}
        ...
        {activeTab === "idioms" && (
          <IdiomsCollocationsList type="idiom" items={items} toggleFavorite={toggleFavorite} openDetail={setDetailItem} showToast={showToast} />
        )}
        {activeTab === "collocations" && (
          <IdiomsCollocationsList type="collocation" items={items} toggleFavorite={toggleFavorite} openDetail={setDetailItem} showToast={showToast} />
        )}
...
function MyVocabulary({ items, toggleFavorite, openDetail, addManualWord, deleteItem, requestConfirm }) {
  ...
  return (
    <div>
      ...
      <div className="vb-card rounded-2xl p-4 mb-5 flex flex-wrap gap-3 items-center">
        ...
        <select className="vb-select" value={difficulty} onChange={(e) => setDifficulty(e.target.value)}>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="vb-card rounded-2xl overflow-hidden">
          <div className="hidden sm:grid grid-cols-[1.5fr_2fr_1fr_1fr_auto_auto_auto] gap-3 px-5 py-3 text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--vb-muted)", borderBottom: "1px solid var(--vb-border)" }}>
            <span>Word</span><span>Meaning</span><span>Part of speech</span><span>Status</span><span>Audio</span><span>Fav</span><span></span>
          </div>
          <div className="divide-y" style={{ borderColor: "var(--vb-border)" }}>
            {filtered.map((v) => (
              <div key={v.id} onClick={() => openDetail(v)} className="grid grid-cols-2 sm:grid-cols-[1.5fr_2fr_1fr_1fr_auto_auto_auto] gap-3 px-5 py-3.5 items-center cursor-pointer vb-row-hover">
      ...
      {showAdd && <AddWordModal onAdd={addManualWord} onClose={() => setShowAdd(false)} />}
    </div>
  );
}

function AddWordModal({ onAdd, onClose }) {
  ...
  const submit = () => {
    if (!canSubmit) return;
    onAdd({
      term: term.trim(), pos, meaning: meaning.trim(),
      example: example.trim() || `I'm learning to use the word "${term.trim()}".`,
      topic: topic.trim() || "My words", difficulty,
    });
    onClose();
  };
      ...
      <div className="vb-card w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-6 sm:p-7">
          <div className="flex items-center justify-between mb-5">
            <h2 className="vb-heading text-xl" style={{ color: "var(--vb-ink)" }}>Add a word manually</h2>
            <button onClick={onClose} className="vb-icon-btn"><X size={18} /></button>
          </div>

          <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>English word or phrase *</label>
          <input value={term} onChange={(e) => setTerm(e.target.value)} className="vb-input w-full mb-4" placeholder="e.g. perseverance" autoFocus />
          ...
          <textarea value={example} onChange={(e) => setExample(e.target.value)} className="vb-input w-full mb-4 resize-none" rows={2} placeholder="e.g. Her perseverance finally paid off." />
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <div>
              ...
              <input value={topic} onChange={(e) => setTopic(e.target.value)} className="vb-input w-full" placeholder="My words" />
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wide mb-1.5" style={{ color: "var(--vb-muted)" }}>Difficulty</label>
              <select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} className="vb-select w-full">
                <option value="easy">Easy</option>
                <option value="medium">Medium</option>
                <option value="hard">Hard</option>
              </select>
...
function ItemDetailModal({ item, onClose, toggleFavorite, updateNotes, onDelete, requestConfirm }) {
...
const FLASHCARD_FILTERS = [
  ...
  { id: "difficult", label: "Difficult words" },
...
function poolForFlashcardFilter(items, filterId) {
  ...
  switch (filterId) {
    ...
    case "difficult": return vocab.filter((v) => v.difficulty === "hard");
...
function PracticeView({ items, gradeItem, recordSentence, quickPool, clearQuickPool, goto }) {
  const [config, setConfig] = useState({ practiceType: "all", questionType: "e2v", difficulty: "all", count: 10 });
  const [session, setSession] = useState(null);
  ...
  const buildPool = () => {
    ...
    if (config.difficulty !== "all") pool = pool.filter((i) => i.difficulty === config.difficulty);
    return pool;
  };

  const start = () => {
    const pool = shuffle(buildPool()).slice(0, config.count);
    ...
    <div>
      ...
      <div className="vb-card rounded-3xl p-6 max-w-xl">
        <PracticeConfigGroup label="What do you want to practice?" options={[
          { id: "all", label: "All" }, { id: "vocab", label: "Vocabulary" }, { id: "idiom", label: "Idioms" }, { id: "collocation", label: "Collocations" },
        ...
        ]} value={config.questionType} onChange={(v) => setConfig((c) => ({ ...c, questionType: v }))} cols={2} />
...
function PracticeConfigGroup({ label, options, value, onChange, cols = 4 }) {
  return (
    <div className="mb-5">
...
function IdiomsCollocationsList({ type, items, toggleFavorite, openDetail, showToast }) {
  const [q, setQ] = useState("");
  ...
  return (
    <div>
      ...
      {list.length === 0 ? (
        <EmptyState icon={Icon} title={`No ${title.toLowerCase()} found`} subtitle="Try a different search term." />
      ) : (
            ...
            <div key={it.id} className="vb-card rounded-2xl p-5 cursor-pointer" onClick={() => openDetail(it)}>
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex items-center gap-2">
                  <DifficultyPill level={it.difficulty} />
                  <button
...
function ImportView({ items, setItems, showToast, goto }) {
  ...
  const [preview, setPreview] = useState(null); // { rows: [{term,pos,meaning,example,dup}], dupChoice }
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      const cols = {
        term: findCol("Vocabulary"), pos: findCol("Part of Speech"),
        meaning: findCol("Vietnamese Meaning"), example: findCol("English Example"),
      };
      const missing = Object.entries(cols).filter(([, idx]) => idx === -1);
      if (missing.length > 0) { setError(`Required columns are missing. Expected: ${TEMPLATE_HEADER.join(", ")}.`); return; }

      const existingTerms = new Set(items.filter((i) => i.type === "vocab").map((i) => normalize(i.term)));
      const dataRows = rows.slice(1)
        .map((r) => ({
          term: String(r[cols.term] || "").trim(),
          pos: String(r[cols.pos] || "").trim(),
          meaning: String(r[cols.meaning] || "").trim(),
          example: String(r[cols.example] || "").trim(),
        }))
        .filter((r) => r.term);

      if (dataRows.length === 0) { setError("No vocabulary rows found — the sheet is empty."); return; }
  ...
  const confirmImport = () => {
    ...
    rows.forEach((r) => {
      ...
      toAdd.push(makeItem("vocab", {
        term: r.term, pos: r.pos || "—", meaning: r.meaning, example: r.example,
        topic: "Imported", difficulty: "medium", source: "import",
      }));
    });

    setItems((prev) => {
      let next = prev;
      if (replacedTerms.length) next = next.filter((it) => !(it.type === "vocab" && replacedTerms.includes(normalize(it.term))));
      return [...next, ...toAdd];
    });

    showToast(`✅ ${toAdd.length} vocabulary item${toAdd.length !== 1 ? "s" : ""} imported successfully!${skipped ? ` (${skipped} duplicate${skipped !== 1 ? "s" : ""} skipped)` : ""}`);
    setPreview(null);
  ...
  return (
    <div>
      <SectionHeader title="Import Vocabulary" subtitle="Upload an Excel file to bulk-add words to your library." right={
      ...
      {!preview && (
        ...
        >
          ...
          <p className="font-medium mb-1" style={{ color: "var(--vb-ink)" }}>📁 Drag & Drop your Excel file here</p>
          ...
          <p className="text-xs mt-4" style={{ color: "var(--vb-muted)" }}>Accepted format: .xlsx</p>
        </div>
      )}
      ...
      {preview && (
        <div className="vb-card rounded-2xl p-6 mt-2">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "var(--vb-mint-pale)" }}>
                  <th className="text-left px-4 py-2.5 font-semibold" style={{ color: "var(--vb-sage-dk)" }}>Vocabulary</th>
                  <th className="text-left px-4 py-2.5 font-semibold" style={{ color: "var(--vb-sage-dk)" }}>Part of Speech</th>
              ...
              <tbody className="divide-y" style={{ borderColor: "var(--vb-border)" }}>
                {preview.rows.slice(0, 50).map((r, i) => (
                  <tr key={i}>
                    ...
                    <td className="px-4 py-2.5" style={{ color: "var(--vb-muted)" }}>{r.pos}</td>
          ...
          <div className="flex gap-2.5 justify-end mt-5">
            ...
            <button onClick={confirmImport} className="vb-btn-primary"><Upload size={15} /> Import Vocabulary</button>
          </div>
        </div>
      )}
    </div>
...
function downloadTemplate() {
  ...
  ws["!cols"] = [{ wch: 18 }, { wch: 16 }, { wch: 36 }, { wch: 46 }];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Vocabulary");
  XLSX.writeFile(wb, "VocaBloom_Vocabulary_Template.xlsx");
}

function TemplateView() {
  return (
    <div>
      <SectionHeader title="Excel Template" subtitle="Download the template, fill in your words, then import them." />
      ...
      <div className="vb-card rounded-3xl p-7 max-w-3xl">
        <div className="flex items-start gap-4 flex-wrap">
          ...
          <div className="flex-1 min-w-[200px]">
            <h3 className="vb-heading text-lg" style={{ color: "var(--vb-ink)" }}>VocaBloom_Vocabulary_Template.xlsx</h3>
            <p className="text-sm mt-1" style={{ color: "var(--vb-muted)" }}>Contains the required columns, in order, with sample rows to guide you.</p>
          </div>
          <button onClick={downloadTemplate} className="vb-btn-primary shrink-0"><Download size={15} /> Download Excel Template</button>
        </div>

        <div className="mt-6">
          <div className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--vb-muted)" }}>Required columns, in this exact order</div>
          <div className="flex flex-wrap gap-2">
            {TEMPLATE_HEADER.map((h, i) => (
        ...
        <div className="mt-6 overflow-x-auto rounded-xl" style={{ border: "1px solid var(--vb-border)" }}>
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: "var(--vb-mint-pale)" }}>
                {TEMPLATE_HEADER.map((h) => <th key={h} className="text-left px-4 py-2.5 font-semibold" style={{ color: "var(--vb-sage-dk)" }}>{h}</th>)}
              </tr>
            </thead>
            <tbody className="divide-y" style={{ borderColor: "var(--vb-border)" }}>
